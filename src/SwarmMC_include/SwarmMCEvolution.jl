...
function HandleDelayEvent(event::DELAY_EVENT_NULL, part::PARTICLE, params::PARAMS)
    error("Shouldn't ever get here!")
end

function HandleDelayEvent(event::DELAY_EVENT_CHANGETYPE, part::PARTICLE, params::PARAMS)
    ChangePType(part, event.new_ptype, params)

    if event.return_vel isa Real
        # return_vel is actually an energy
        part.vel = GenerateInitVel(PIVS_ISOTROPIC(event.return_vel), event.new_ptype) 
    elseif event.return_vel isa Function
        part.vel = event.return_vel()
    else
        error("Unknown type of event.return_vel = $(event.return_vel).")
    end

    UpdateEpsBin(part, params.eps_bin_grid)

    #println("Returned with eps=$(part.eps)")

    nothing
end

function HandleDelayEvent(event::DELAY_EVENT_LOSS, part::PARTICLE, params::PARAMS)
    part.weight = 0
...
# function HandleRegionEvent(event::REGION_EVENT_NULL, part::PARTICLE, region::REGION, params::PARAMS, props_out::PROPS_OUT)
# end

# function HandleRegionEvent(event::REGION_EVENT_DIE, part::PARTICLE, region::REGION, params::PARAMS, props_out::PROPS_OUT)
#     part.weight = 0.
# end

# function HandleRegionEvent(event::REGION_EVENT_MEASURE, part::PARTICLE, region::REGION, params::PARAMS, props_out::PROPS_OUT)
#     thisprops = INT_PROPS(params.num_cf[part.ptype_ind], params.meas_num_l)

#     FillIntPropsWithInstMeas!(thisprops, part, params.do_extra_meas, params.meas_num_l)

#     meas_ind = MakeIndices(event.meas_type, part.t_bin, part.r_bin, part.z_bin, part.eps_bin)

#     #event.meas[part.ptype_ind, meas_ind...] += thisprops
#     props_out.props[event.meas_ind][2][part.ptype_ind, meas_ind...] += thisprops
...
# function FillIntPropsWithInstMeas!(int_props::INT_PROPS, part::PARTICLE, do_extra_meas::TypeBool, meas_num_l::Int)
#     int_props.log2fac = part.log2fac

#     pos = part.pos
#     vel = part.vel
#     eps = part.eps

#     weight_prefac = part.weight
#     int_props.noweight =    1.
#     int_props.weight =      weight_prefac
#     # int_props.weightsqr =   weight_prefac * weight_prefac
#     # int_props.weightcube =  weight_prefac * weight_prefac^2
#     # int_props.time =        weight_prefac * part.time
#     int_props.pos =         weight_prefac * pos
#     int_props.possqr =      weight_prefac * pos.*pos
#     int_props.vel =         weight_prefac * vel
#     # int_props.velsqr =      weight_prefac * vel.*vel
#     int_props.posvel =      weight_prefac * pos.*vel
#     int_props.eps =         weight_prefac * eps
#     # int_props.epssqr =      weight_prefac * eps*eps
#     # int_props.poseps =      weight_prefac * pos * eps
#     # int_props.epsflux =     weight_prefac * vel * eps
#     # int_props.epspressure = weight_prefac * vel.*vel * eps

#     if IsTrue(do_extra_meas)
#         int_props.weightsqr =   weight_prefac * weight_prefac
#         int_props.weightcube =  weight_prefac * weight_prefac^2
#         int_props.time =        weight_prefac * part.time
#         int_props.velsqr =      weight_prefac * vel.*vel
...
#         costh = vel[3] / sqrt(sum(vel.^2))

#         int_props.dist_legendre = weight_prefac * Pl(costh, lmax=meas_num_l-1).parent
#         # for ind in 1:meas_num_l
#         #     l = ind-1
#         #     int_props.dist_legendre[ind] = weight_prefac * LegendrePoly(l)(costh)
#         # end

#         int_props.costh = weight_prefac * costh.^(1:meas_num_l)
#     end
#     nothing
# end
